...
class TwarkovChain(MarkovChain):
  ...
  def _ProcTweet(self, tweet, familiar=None):
    words = self._Tokenize(tweet.text)
  ...
  def GetMessage(self, seed=None, max_len=140,
                 depth=None, sep=" ", trunc_char=None, labelset=None,tweetset=None):
    ...
    Arguments:
      ...
      tweetset: If provided, assumed to be an empty set to be populated with
        all the tweet id's used in constructing the message.
...
class CharChain(TwarkovChain):
  ...
  def GetMessage(self, seed=None, max_len=140,
                 depth=None, sep='', trunc_char=" ", tweetset=None):
    return TwarkovChain.GetMessage(self, seed, max_len, depth, sep, trunc_char, tweetset=tweetset)
